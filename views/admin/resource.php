    ...
    <form id="resource-form" method="post" action="/admin/<?= e($collection) ?>/save" class="admin-form" enctype="multipart/form-data">
        ...
        <div class="admin-form__row" style="grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));">
            <?php foreach($fields as $field): ?>
                <label><?= e(ucwords(str_replace('_',' ',$field))) ?>
                    <?php if(in_array($field, ['description','subheadline','body','items','profile','address','joining_note'], true)): ?>
        ...
        <?php if(in_array($collection, ['events','speakers','venues','publishers'], true)): ?>
            <div class="admin-upload-panel">
                ...
                <p>Uploaded files are saved into the media library. Event, speaker, and venue uploads can fill the matching image field when empty.</p>
                <div class="admin-image-preview" id="media-image-preview"></div>
            </div>
...
const mediaImageInput = document.getElementById('field-media-files');
if (mediaImageInput) {
    mediaImageInput.addEventListener('change', () => {
        const preview = document.getElementById('media-image-preview');
        if (!preview) return;
        preview.innerHTML = '';
        [...mediaImageInput.files].forEach(file => {
