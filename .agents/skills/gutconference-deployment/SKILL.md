...
- Before comparing or rebasing for release, ensure `origin` is `https://github.com/getwinharris/GutConference-phpCMS.git`, run `git fetch origin main`, and use the freshly fetched `origin/main` / `FETCH_HEAD` as the remote baseline instead of assuming the local tracking ref is current.
