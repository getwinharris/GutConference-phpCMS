...
final class AdminController extends BaseController {
    ...
    public function saveAdminCredentials(): void{(new EnvService())->saveAdminCredentials($_POST); $this->flash('Admin credentials saved to encrypted integrations.'); $this->redirect('/admin/settings');}
    ...
    public function environment(): void{$this->render('admin/environment',['pageTitle'=>'Environment','permissions'=>(new StoragePermissionService())->status(),'geminiDiagnostics'=>(new GeminiModelRouter())->diagnostics()]);}
    public function saveEnvironment(): void{(new AuditLogService())->record('blocked','environment','project-env-files-disabled'); $this->flash('Project env files are disabled. Use Integrations for secrets and server-level environment configuration outside the repo.'); $this->redirect('/admin/environment');}
