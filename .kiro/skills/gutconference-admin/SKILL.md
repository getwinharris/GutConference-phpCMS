...
description: GutConference Admin Skill. Use when editing GutConference owner/admin pages, CRUD forms, media library, environment editor, permissions, audit log, integrations, admin secrets, automation, admin navigation, or admin support-agent surfaces.
---
...
- Preserve visible admin text/content meaning during UI-only refactors unless the user explicitly requests copy changes.
...
- Admin integration secrets must be saved through `SecretService` into encrypted `storage/data/adminsecrets.json`; keep secret files git-ignored and out of public views.
