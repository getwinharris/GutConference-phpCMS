...
foreach (['/','/signup','/auth/google','/auth/google/callback','/events','/events/{slug}','/admin/events','/admin/event_sections','/admin/speakers','/admin/sessions','/admin/venues','/admin/publishers','/admin/registrations','/admin/notification_templates','/admin/support-tickets'] as $path) {
...
assertTrue(str_contains($event, 'Join Now') && str_contains($event, 'Pay with Razorpay'), 'event ticket flow gates payment behind login and direct Razorpay');
...
assertTrue(str_contains($event, 'E-certificate included') && !str_contains($event, 'conference-note') && !str_contains($event, 'agenda slots'), 'event page uses simplified official brief content');
assertTrue(str_contains($admin, '/admin/events') && str_contains($admin, '/admin/venues') && str_contains($admin, '/admin/publishers'), 'admin nav exposes conference resources');
