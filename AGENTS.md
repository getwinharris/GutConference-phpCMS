...
3. `docs/PROJECT_MAP.md` (route list) and `docs/systematic-map.md` (full connection map, gaps, schema vs storage, collection usage)
4. `example-Agent.md`
...
The project map is generated by `tools/generate-project-map.php` and is the single source of truth for what is wired in the codebase. It produces:

- `docs/project-map.json` — the route registry (used by `tools/validate-project-map.php`).
- `docs/PROJECT_MAP.md` — the route list.
- `docs/project-map.mmd` — the route graph (mermaid).
- `docs/systematic-map.json` — the full connection scan (controllers, services, views, integrations, schema collections, storage data files, collection usage, gaps). Stable; the validator compares the registry to this byte-for-byte.
- `docs/systematic-map.md` — the human-readable systematic map with a "Gaps" section sorted by severity.
- `docs/systematic-map.mmd` — the full mermaid diagram. Subgraphs: routes by domain (PUBLIC, AUTH, PAYMENT, SUPPORT, ADMIN), Controllers, Services, Views, Integrations, Schema collections, Storage data files, Tools, and a Gaps subgraph. Every node shows where it is and what calls it. Dashed edges mark unwired/missing links (red gap nodes).
...
- **Before writing a GitHub issue.** Read the relevant `systematic-map.md` gaps section and quote the lines that justify the issue. Issues must be backed by the systematic map, not by guesswork.
...
1. Open `docs/systematic-map.md` and find the relevant section (controllers, services, schema, gaps, collection usage).
2. Quote the exact gap line that proves the issue is real. If no gap is listed, the issue is not validated yet — investigate the code first.
3. The fix outline must reference existing files and existing services where possible. Do not invent new services / controllers / views when the map shows existing code already covers the case.
